...
class BrandController extends Controller
{
    public function index(ApiService $api)
    {
        try {
            ...
            return back()->with('error', 'Server Brand tidak bisa dihubungi');
        }
    }

    public function downloadTemplate()
    {
        $fileName = 'template_import_brand.xlsx';
        ...
        // Header columns
        $sheet->setCellValue('A1', 'Nama Brand');
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                ...
                try {
                    ...
                    $api->request('post', '/brands', $payload);
                    ...
                    Log::error('Import Brand Row Error', [
                        'row' => $index + 1,
                        'error' => $e->getMessage()
                    ]);
                }
            ...
            Log::error('Import Brand System Error: ' . $e->getMessage());
    ...
    public function update(Request $r, $id)
    {
        try {
            ...
            $response = Http::withToken($token)
                ->patch(env('API_END_POINT') . "/brands/{$id}", $payload);
    ...
    public function show($id, ApiService $api)
    {
        try {
            ...
            $result = $api->request('get', "/brands/$id");
    ...
    public function destroy($id)
    {
        try {
            ...
            $response = Http::withToken(session('accessToken'))
                ->delete(env('API_END_POINT') . "/brands/{$id}");

            if ($response->failed()) {
                return response()->json([
                    'server' => 'Gagal menghapus Brand dari server'
                ], 500);
            }
